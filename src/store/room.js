...
export default {
  ...
  mutations: {
    ...
    popGameHistories(state, { history = null } = {}) {
      ...
      if (history) {
        state.gameHistories = state.gameHistories.filter(
          (h) => h.id !== history.id
        );
        lastHistory = history;
      } else {
      ...
      if (command === "moveCards") {
        ...
        if (oldState.fromCardGropus) {
          state.players[player]["cards"][ZONE_GROUP_MAP[from]] =
      ...
      if (command === "groupCard") {
        state.players[player]["cards"][from] = oldState.fromCards;
        state.players[player]["cards"][to] = oldState.toCards;
        state.players[player]["cards"][ZONE_GROUP_MAP[to]] =
    ...
    groupCard(state, { from, to, fromCard, toCard, player }) {
      // 正当な宛先かチェック
      ...
      // 変更前の状態を保持する。
      const oldState = {
        fromCards: JSON.parse(
          JSON.stringify(state.players[player]["cards"][from])
        ),
        toCards: JSON.parse(JSON.stringify(state.players[player]["cards"][to])),
        toCardGroups: JSON.parse(
          JSON.stringify(state.players[player]["cards"][ZONE_GROUP_MAP[to]])
        ),
      };
      // 移動元からカードを削除する。
    ...
    moveCards(state, { from, to, cards, player, prepend = false }) {
      // 変更前の状態を保持する。
      const oldState = {
        fromCards: JSON.parse(
          JSON.stringify(state.players[player]["cards"][from])
      ...
      if (card.groupId) {
        ...
        // 状態を保持する。
        oldState.fromCardGropus = JSON.parse(
          JSON.stringify(state.players[player]["cards"][ZONE_GROUP_MAP[from]])
        );
    ...
    changeCardsState(state, { from, cards, player, cardState }) {
      const oldState = {
        fromCards: JSON.parse(
          JSON.stringify(state.players[player]["cards"][from])
        ),
