<template>
  <div class="deck-header">
    <div>
      <label>
        <input type="radio" value="redyMade" :name="'deckType' + side" v-model="selected.deckType" />
        <span>構築済み</span>
      </label>
      <label style="margin-left: 10px;">
        <input type="radio" value="custom" :name="'deckType' + side" v-model="selected.deckType" />
        <span>ユーザー作成</span>
      </label>
    </div>
    <div class="deck-select">
      <select name="deckId" id="select-deck" @change="changeDeck" v-model="selected.index">
        <option
    ...
    <Modal class="deck-header-modal" v-if="modal.update" @close-modal="modal.update = false">
    ...
    <Modal class="deck-header-modal" v-if="modal.create" @close-modal="modal.create = false">
      <template v-slot:content>
        <div>
          <p>デッキの名前を入力してください</p>
        </div>
...
export default {
  ...
  methods: {
    ...
    updateDeckName() {
      this.$parent.deckList[this.selected.deckType][this.selected.index].name = this.params.name;
      this.updateDeck();
